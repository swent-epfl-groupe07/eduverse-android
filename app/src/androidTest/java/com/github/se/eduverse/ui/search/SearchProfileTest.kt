...
class SearchProfileScreenTest {
    ...
    fun searchField_isDisplayed() {
        composeTestRule.setContent {
            SearchProfileScreen(viewModel = viewModel, onProfileClick = {})
        }

        composeTestRule
            .onNodeWithTag(TAG_SEARCH_FIELD)
            .assertExists()
            .assertIsDisplayed()
    }
    ...
    fun successState_withResults_showsProfiles() {
        ...
        composeTestRule
            .onNodeWithTag(TAG_PROFILE_LIST)
            .assertExists()
            .assertIsDisplayed()
    ...
    fun profileClick_triggersCallback() {
        ...
        composeTestRule.setContent {
            SearchProfileScreen(
                viewModel = viewModel,
                onProfileClick = { clickedProfileId = it }
            )
        }
...
class ProfileSearchItemTest {
    ...
    fun allProfileElements_areDisplayed() {
        composeTestRule.setContent {
            ProfileSearchItem(profile = testProfile, onClick = {})
        }
        ...
        composeTestRule
            .onNodeWithTag("${TAG_PROFILE_STATS}_1")
            .assertExists()
            .assertIsDisplayed()

        composeTestRule
            .onNodeWithTag("${TAG_PROFILE_IMAGE}_1")
            .assertExists()
            .assertIsDisplayed()
    }
    ...
    fun click_triggersCallback() {
        ...
        assert(wasClicked) {
            "Click callback was not triggered"
        }
    }
    ...
    fun longUsername_isEllipsized() {
        val longNameProfile = testProfile.copy(
            username = "ThisIsAVeryLongUsernameThatShouldBeEllipsized"
        )

        composeTestRule.setContent {
            ProfileSearchItem(profile = longNameProfile, onClick = {})
        }

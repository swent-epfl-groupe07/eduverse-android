...
class FakeFolderViewModel(private val folderRepository: FolderRepository, auth: FirebaseAuth) :
  ...
  override fun addFolder(folder: Folder) {
    ...
    folderRepository.addFolder(
        ...
        { error -> println("DEBUG: Failed to add folder: $error") })
  }
  ...
  override fun updateFolder(folder: Folder) {
    ...
    folderRepository.updateFolder(
        folder,
        {
          val updatedFolders = foldersList.map { if (it.id == folder.id) folder else it }
          foldersList.clear()
  ...
  override fun goBack() {
    ...
    val newRoute =
        when (_currentRoute.value) {
          ...
          "FOLDER" ->
              "LIST_FOLDERS" // Only go to list when explicitly navigating back from folder view
...
fun TestFolderNavigation(
  ...
  when (currentScreen) {
    "LIST_FOLDERS" ->
        ListFoldersScreen(navigationActions = navigationActions, folderViewModel = folderViewModel)
    "CREATE_FOLDER" ->
        CreateFolderScreen(
            navigationActions = navigationActions,
            folderViewModel = folderViewModel,
    ...
    "FOLDER" ->
        FolderScreen(
            ...
            fileViewModel = fileViewModel)
    "CREATE_FILE" ->
        CreateFileScreen(navigationActions = navigationActions, fileViewModel = fileViewModel)

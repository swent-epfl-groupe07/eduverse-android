...
class DashboardRepositoryImpl(
    ...
    override fun getWidgets(userId: String): Flow<List<Widget>> = callbackFlow {
        val widgetRef = firestore.collection("users").document(userId).collection("widgets")
        val listener = widgetRef.addSnapshotListener { snapshot, e ->
    ...
    override suspend fun removeWidget(userId: String, widgetId: String) {
        val widgetRef = firestore.collection("users").document(userId).collection("widgets").document(widgetId)
        widgetRef.delete()
    }
}
...
interface ProfileRepository {
  ...
  suspend fun searchProfilesByType(type: ProfileType): List<Profile>

  suspend fun searchNearbyTeachers(userLocation: Location, radius: Double): List<Profile>
}

open class ProfileRepositoryImpl(
    private val firestore: FirebaseFirestore,
    private val storage: FirebaseStorage,
    private val nominatimRepository: NominatimLocationRepository
) : ProfileRepository {
    ...
    override suspend fun getProfile(userId: String): Profile? {
        ...
        // Get publications
        val publications =
            publicationsCollection.whereEqualTo("userId", userId).get()
        ...
        val followersCount =
            followersCollection.whereEqualTo("followedId", userId).get().await().size()

        val followingCount =
            followersCollection.whereEqualTo("followerId", userId).get().await().size()
            ...
            profilesCollection
                ...
                .mapNotNull { doc ->
                    ...
                    profile?.let {
                        ...
                        val followersCount =
                            followersCollection.whereEqualTo("followedId", it.id).get().await()
                                .size()

                        val followingCount =
                            followersCollection.whereEqualTo("followerId", it.id).get().await()
                                .size()
                ...
                firestore
                    .runTransaction { transaction ->
                        ...
                        if (!likedBy.contains(userId)) {
                            ...
                            Log.d(
                                "INCREMENTCHECK",
                                "User $userId has already liked this publication."
                            )
                        }
                    }
                    .await()
    ...
    override suspend fun removeFromLikedPublications(userId: String, publicationId: String) {
        try {
            ...
            Log.d(
                "REMOVE_LIKE",
                "Failed to remove publication from likedPublications: ${e.message}"
            )
            throw e
        }
    }
    ...
    override suspend fun decrementLikesAndRemoveUser(publicationId: String, userId: String) {
        try {
            ...
            if (!querySnapshot.isEmpty) {
                ...
                firestore
                    .runTransaction { transaction ->
                        val snapshot = transaction.get(documentRef)
                        val likedBy =
                            snapshot.get("likedBy") as? MutableList<String> ?: mutableListOf()
                        ...
                        if (likedBy.contains(userId)) {
                            ...
                            transaction.update(
                                documentRef,
                                mapOf("likedBy" to likedBy, "likes" to newLikes)
                            )
                        }
                    }
                    .await()
            ...
            Log.d(
                "REMOVE_LIKE",
                "Failed to decrement likes and remove user from likedBy: ${e.message}"
            )
            throw e
        }
    }
    ...
    override suspend fun searchProfilesByType(type: ProfileType): List<Profile> {
        return try {
            profilesCollection
                .whereEqualTo("profileType", type.name) // Utilisation du champ profileType
                .get()
                .await()
                .documents
                .mapNotNull { it.toObject(Profile::class.java) }
        } catch (e: Exception) {
            Log.e("SEARCH_BY_TYPE", "Failed to search profiles by type: ${e.message}")
            emptyList()
        }
    }


    override suspend fun searchNearbyTeachers(
        userLocation: Location,
        radius: Double
    ): List<Profile> {
        return try {
            profilesCollection
                .whereEqualTo("profileType", ProfileType.TEACHER.name)
                .get()
                .await()
                .documents
                .mapNotNull { it.toObject(Profile::class.java) }
                .map { profile ->
                    val location = profile.address?.let { nominatimRepository.parseLocation(it) } // Utilise le repository
                    profile to location
                }
                .filter { (_, location) ->
                    location != null && isNearby(userLocation, location, radius)
                }
                .map { (profile, _) -> profile }
        } catch (e: Exception) {
            Log.e("NEARBY_TEACHERS", "Failed to search nearby teachers: ${e.message}")
            emptyList()
        }
    }
    ...
    private fun isNearby(location1: Location, location2: Location, radius: Double): Boolean {
